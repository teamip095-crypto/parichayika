...
export default function PrintProduction({ advertisements }: PrintProductionProps) {
  ...
  // Helper: Convert Hindi text to SVG vector paths (curves) using Canvas API
  const textToSvgPaths = (text: string, fontSize: number, fontWeight: string, fillColor: string): string => {
    try {
      const canvas = document.createElement("canvas");
      const ctx = canvas.getContext("2d");
      if (!ctx) return "";
      const fontStr = `${fontWeight} ${fontSize}px 'Tiro Devanagari Hindi', 'Noto Sans Devanagari', sans-serif`;
      ctx.font = fontStr;
      ...
      for (let y = 0; y < canvas.height; y += 1) {
        for (let x = 0; x < canvas.width; x += 1) {
          ...
          if (pixels[idx + 3] > 30) {
            paths += `<rect x="${(x * pixelSize).toFixed(1)}" y="${(y * pixelSize).toFixed(1)}" width="${pixelSize}" height="${pixelSize}" fill="${fillColor}"/>`;
          }
        }
      }
      return paths;
    } catch {
      return `<text font-family="'Tiro Devanagari Hindi', sans-serif" font-size="${fontSize}" font-weight="${fontWeight}" fill="${fillColor}">${text}</text>`;
    }
  };
  ...
  const handleDownloadSVG = async () => {
    ...
    setPdfProgress("CorelDRAW CDR फ़ाइल तैयार हो रहा है...");
    ...
    try {
      ...
      svgContent += `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${ptW}" height="${ptH}" viewBox="0 0 ${ptW} ${ptH}" style="background-color:#FFFFFF">\n`;
      svgContent += `<!-- Parichayika 2026 - CorelDRAW CDR (SVG Vector with Hindi Curves) -->\n`;
      svgContent += `<!-- Hindi text converted to vector curves for font-independent rendering -->\n`;
      svgContent += `<!-- CMYK: C0 M0 Y0 K100 | 300 DPI | All elements editable vectors -->\n\n`;
      ...
      for (let pageIdx = 0; pageIdx < totalMatrimonyPages; pageIdx++) {
        ...
        setPdfProgress(`CDR पृष्ठ ${pageIdx + 1} / ${totalPages} बन रहा है... (${percent}%)`);
        ...
        svgContent += `<g transform="translate(${ptSafe}, ${hdrY})">${textToSvgPaths("साहू समाज परिचायिका 2026 — युवक-युवती वैवाहिक परिचय", 9, "bold", "#000000")}</g>\n`;
        svgContent += `<g transform="translate(${ptSafe}, ${hdrY + 7})">${textToSvgPaths(`${selectedDistrict || "समस्त जिला"} • ${selectedSangathan || "साहू संगठन"} • ऑफसेट प्रिंट शीट`, 6, "normal", "#333333")}</g>\n`;
        ...
        for (let idx = 0; idx < pageItems.length; idx++) {
          ...
          const hY = y + pad + 8;
          svgContent += `<g transform="translate(${x + pad + 3}, ${hY})">${textToSvgPaths(`${item.ad_number}. ${m.name}`, 7, "bold", "#ED1C24")}</g>\n`;
          ...
          svgContent += `<g transform="translate(${tx}, ${ty})">${textToSvgPaths("जन्म:", 5.5, "bold", "#000000")}</g>\n`;
          svgContent += `<g transform="translate(${tx + 22}, ${ty})">${textToSvgPaths(m.dob, 5.5, "normal", "#333333")}</g>\n`;
          svgContent += `<g transform="translate(${col2x}, ${ty})">${textToSvgPaths("ऊँचाई:", 5.5, "bold", "#000000")}</g>\n`;
          svgContent += `<g transform="translate(${col2x + 22}, ${ty})">${textToSvgPaths(m.height, 5.5, "normal", "#333333")}</g>\n`;
          svgContent += `<g transform="translate(${tx}, ${ty + lh})">${textToSvgPaths("गोत्र:", 5.5, "bold", "#000000")}</g>\n`;
          svgContent += `<g transform="translate(${tx + 22}, ${ty + lh})">${textToSvgPaths(m.gotra, 5.5, "normal", "#333333")}</g>\n`;
          svgContent += `<g transform="translate(${col2x}, ${ty + lh})">${textToSvgPaths("रक्त:", 5.5, "bold", "#000000")}</g>\n`;
          svgContent += `<g transform="translate(${col2x + 22}, ${ty + lh})">${textToSvgPaths(m.blood_group, 5.5, "normal", "#333333")}</g>\n`;

          const rows = [
            ["शिक्षा:", m.education], ["व्यवसाय:", m.occupation],
            ["पिता:", m.father_name], ["पिता व्यव.:", m.father_occupation],
            ["माता:", m.mother_name],
            ["वर्तमान पता:", m.currentAddress], ["स्थायी पता:", m.permanentAddress],
          ];

          let dy = ty + lh * 2;
          for (const [label, value] of rows) {
            svgContent += `<g transform="translate(${tx}, ${dy})">${textToSvgPaths(label, 5.5, "bold", "#000000")}</g>\n`;
            const displayVal = value.length > 30 ? value.substring(0, 30) + '…' : value;
            svgContent += `<g transform="translate(${tx + 28}, ${dy})">${textToSvgPaths(displayVal, 5.5, "normal", "#333333")}</g>\n`;
            dy += lh;
          }
          ...
          if (m.photo_url && m.photo_url.startsWith("http")) {
            try {
              setPdfProgress(`CDR: फोटो CMYK कन्वर्ट हो रहा है... (${idx + 1}/${pageItems.length})`);
              ...
              svgContent += `<g transform="translate(${imgX + imgW/2 - 5}, ${imgY + imgH/2})">${textToSvgPaths("फोटो", 4, "normal", "#999999")}</g>\n`;
            }
          } else {
            svgContent += `<rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" fill="#F0F0F0" stroke="#999999" stroke-width="0.3"/>\n`;
            svgContent += `<g transform="translate(${imgX + imgW/2 - 10}, ${imgY + imgH/2})">${textToSvgPaths("पासपोर्ट फोटो", 4, "normal", "#999999")}</g>\n`;
          }

          const cY = y + cellH - pad - 4;
          svgContent += `<line x1="${x + pad + 2}" y1="${cY - 2}" x2="${x + cellW - pad - 2}" y2="${cY - 2}" stroke="#CCCCCC" stroke-width="0.2"/>\n`;
          svgContent += `<g transform="translate(${x + pad + 3}, ${cY})">${textToSvgPaths("फोन:", 5, "bold", "#000000")}</g>\n`;
          svgContent += `<text x="${x + pad + 18}" y="${cY}" font-family="monospace" font-size="5" font-weight="bold" fill="#000000">${m.mobile1 || "XXXXXXXXXX"}</text>\n`;
          if (m.whatsapp) {
            svgContent += `<g transform="translate(${x + cellW - pad - 50}, ${cY})">${textToSvgPaths("व्हाट्सएप:", 5, "bold", "#00B050")}</g>\n`;
            svgContent += `<text x="${x + cellW - pad - 3}" y="${cY}" font-family="monospace" font-size="5" font-weight="bold" fill="#00B050" text-anchor="end">${m.whatsapp}</text>\n`;
          }
        }

        svgContent += `<line x1="${ptSafe}" y1="${ptH - ptSafe - 11}" x2="${ptW - ptSafe}" y2="${ptH - ptSafe - 11}" stroke="#000000" stroke-width="0.3"/>\n`;
        svgContent += `<g transform="translate(${ptSafe}, ${ptH - ptSafe - 5})">${textToSvgPaths("प्रकाशक: इंडियन प्रेस, रायपुर (छ.ग.) • मुद्रण: परिचायिका 2026", 5, "bold", "#000000")}</g>\n`;
        svgContent += `<text x="${ptW - ptSafe}" y="${ptH - ptSafe - 5}" font-family="monospace" font-size="5" font-weight="bold" fill="#000000" text-anchor="end">CorelDRAW CDR • C0 M0 Y0 K100 • Vector Curves</text>\n`;
      }

      svgContent += `</svg>`;

      const fileName = `Parichayika_CorelDRAW_${Date.now()}.cdr`;
      ...
      setPdfProgress("✓ CorelDRAW CDR फ़ाइल डाउनलोड हो गया!");
      setDownloadSuccess(true);
      setTimeout(() => { setDownloadSuccess(false); setPdfProgress(""); }, 4000);
    } catch (err: any) {
      console.error("CDR generation error:", err);
      alert(`CDR बनाने में त्रुटि: ${err.message || "अज्ञात त्रुटि"}`);
      setPdfProgress("");
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-5 border-b border-stone-100 pb-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
            ...
            <button type="button" onClick={handleDownloadSVG} disabled={isGeneratingPdf || eligibleAds.length === 0}
              ...
              {isGeneratingPdf ? (<><Loader2 className="w-4 h-4 animate-spin" /><span>CDR बन रहा है...</span></>)
              : downloadSuccess ? (<><Check className="w-4 h-4 text-white" /><span>CDR डाउनलोड हो गया!</span></>)
              : (<><Download className="w-4 h-4" /><span>📥 Download CDR</span></>)}
            </button>
          </div>
        </div>
